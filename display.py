...
def render(screen, all_creatures, food, camera, screen_width, screen_height, area_size):
    """
    ...
    :param area_size: размер зоны действий.
    """

    screen.fill((0, 0, 0))
    pg.draw.rect(screen, (255, 0, 0), [((-area_size/2+10) - camera.x) * camera.scale + screen_width / 2,
                                       ((-area_size/2+10) - camera.y) * camera.scale + screen_height / 2,
                                       (area_size-20) * camera.scale + 1,
                                       (area_size-20) * camera.scale + 1])
    # Отрисовка еды
...
class Camera:
    ...
    def move(self, screen_width, screen_height, area_size):
        """
        ...
        if not ((kpressed[pg.K_UP] or kpressed[pg.K_w]) and (kpressed[pg.K_DOWN] or kpressed[pg.K_s])):
            ...
            if kpressed[pg.K_UP] or kpressed[pg.K_w]:
                ...
                if self.y - (screen_height / self.scale) / 2 >= -area_size / 2 + 10 / self.scale:
                    self.y -= 10 / self.scale   # Делим на масштаб, чтобы при меньшем масштабе камера двигалась быстрее
                else:
                    self.y = (screen_height / self.scale) / 2 - area_size / 2
                ...
                if self.y + (screen_height / self.scale) / 2 <= area_size / 2 - 10 / self.scale:
                    ...
                    self.y = area_size / 2 - (screen_height / self.scale) / 2
        ...
        if not ((kpressed[pg.K_LEFT] or kpressed[pg.K_a]) and (kpressed[pg.K_RIGHT] or kpressed[pg.K_d])):
            ...
            if kpressed[pg.K_LEFT] or kpressed[pg.K_a]:
                ...
                if self.x - (screen_width / self.scale) / 2 >= -area_size / 2 + 10 / self.scale:
                    self.x -= 10 / self.scale
                else:
                    self.x = (screen_width / self.scale) / 2 - area_size / 2
                ...
                if self.x + (screen_width / self.scale) / 2 <= area_size / 2 - 10 / self.scale:
                    ...
                    self.x = area_size / 2 - (screen_width / self.scale) / 2
    ...
    def scale_minus(self, screen_width, screen_height, area_size):
        ...
        :param screen_width: ширина экрана.
        :param screen_height: высота экрана.
        :param area_size: ширина зоны действий.
        """

        scale_candidate = self.scale / 1.3

        # Если можно свободно отдаляться без выхода из зоны действий
        if all([((self.x - screen_width / 2 / scale_candidate) >= -area_size / 2),
                ((self.x + screen_width / 2 / scale_candidate) <= area_size / 2),
                ((self.y - screen_height / 2 / scale_candidate) >= -area_size / 2),
                ((self.y + screen_height / 2 / scale_candidate) <= area_size / 2)]):
            ...
            if all([((self.x - screen_width / 2 / self.scale) != -area_size / 2),
                    ((self.x + screen_width / 2 / self.scale) != area_size / 2),
                    ((self.y - screen_height / 2 / self.scale) != -area_size / 2),
                    ((self.y + screen_height / 2 / self.scale) != area_size / 2)]):

                self.scale = max(screen_width / (2 * self.x + area_size), screen_width / (area_size - 2 * self.x),
                                 screen_height / (2 * self.y + area_size), screen_height / (area_size - 2 * self.y))
            else:
                ...
                if any([(old_x - screen_width / 2 / old_scale) == -area_size / 2,
                        (old_x + screen_width / 2 / old_scale) == area_size / 2]):
                    scale_candidate = max(old_scale / 1.3, screen_width / area_size,
                                          screen_height / (2 * (old_y + area_size / 2)) * (old_y <= 0) *
                                          ((old_y - screen_height / 2 / old_scale) != -area_size / 2),
                                          screen_height / (2 * (area_size / 2 - old_y)) * (old_y >= 0) *
                                          ((old_y + screen_height / 2 / old_scale) != area_size / 2))

                # Если камера около верхней или нижней стенки
                if any([(old_y - screen_height / 2 / old_scale) == -area_size / 2,
                        (old_y + screen_height / 2 / old_scale) == area_size / 2]):
                    scale_candidate = max((old_scale / 1.3), (screen_height / area_size),
                                          (screen_width / (2 * (old_x + area_size / 2)) * (old_x <= 0) *
                                          ((old_x - screen_width / 2 / old_scale) != -area_size / 2)),
                                          (screen_width / (2 * (area_size / 2 - old_x)) * (old_x >= 0) *
                                          ((old_x + screen_width / 2 / old_scale) != area_size / 2)),
                                          scale_candidate)
                self.scale = scale_candidate

                # Перемещение камеры от стенки
                if (self.x - screen_width / 2 / old_scale) == -area_size / 2:
                    self.x = (screen_width / self.scale - area_size) / 2

                elif (old_x + screen_width / 2 / old_scale) == area_size / 2:
                    self.x = (area_size - screen_width / self.scale) / 2

                if (self.y - screen_height / 2 / old_scale) == -area_size / 2:
                    self.y = (screen_height / self.scale - area_size) / 2
